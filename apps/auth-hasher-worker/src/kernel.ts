import kernelModule from './rust-wasm-kernel.wasm';
...
const getKernel = (): Promise<RustKernelExports> => {
  if (!kernelPromise) {
    kernelPromise = WebAssembly.instantiate(kernelModule, {}).then(
      (instance) => instance.exports as unknown as RustKernelExports,
    );
  }
