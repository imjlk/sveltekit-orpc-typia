...
공개 전 체크리스트: [`docs/public-release.md`](./docs/public-release.md)
...
- 기본 관계형 데이터 경로인 `Drizzle` + `D1`, 그리고 선택 가능한 Hyperdrive/Postgres API 런타임
...
flowchart LR
    ...
    API --> APP
    APP --> API_DB["D1 또는 Hyperdrive Postgres / Drizzle"]
...
1. D1 데이터베이스를 만들고 반환된 `database_name`, `database_id` 를 [apps/web/wrangler.jsonc](./apps/web/wrangler.jsonc) 에 반영합니다.

```bash
bunx wrangler d1 create <your-d1-name>
```

2. [apps/web/wrangler.jsonc](./apps/web/wrangler.jsonc) 의 `BETTER_AUTH_URL` 을 실제 Pages 도메인으로 바꿉니다.
...
6. [apps/web/wrangler.jsonc](./apps/web/wrangler.jsonc) 의 `AUTH_HASHER` service binding 이 `cloudflare-first-starter-auth-hasher` 를 가리키는지 유지합니다.
...
[apps/web/wrangler.jsonc](./apps/web/wrangler.jsonc) 에 체크인된 `HYPERDRIVE` 바인딩은 `localConnectionString` 기반 로컬 Postgres용 설정입니다. Cloudflare에서 사용할 때는 `id` 값을 실제 Hyperdrive config id로 교체하세요. `ORPC_DB_DRIVER=hyperdrive` 일 때 in-process `/rpc`, `/api` gateway는 `@repo/db/postgres` 와 `packages/db/drizzle-pg/` 의 Postgres migration을 사용합니다.
로컬 Postgres schema 초기화는 아래처럼 실행합니다.
...
flowchart LR
    ...
    APP --> DB["D1 또는 Hyperdrive Postgres"]
...
- 저장소를 public 또는 GitHub template으로 열기 전에 [`docs/public-release.md`](./docs/public-release.md) 체크리스트 확인하기
